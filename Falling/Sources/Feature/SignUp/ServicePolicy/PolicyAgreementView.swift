...
final class PolicyAgreementView: TFBaseView {
	private lazy var logoView: UIImageView = UIImageView().then {
		$0.image = FallingAsset.Image.fallingLogo.image
	}
	
	lazy var selectAllBtn: UIButton = UIButton().then {
		$0.setImage(FallingAsset.Image.checkCir.image, for: .normal)
...
@available(iOS 13.0, *)
struct PolicyAgreementView_Preview: PreviewProvider {
		static var previews: some View {
				UIViewPreview {
						PolicyAgreementView(frame: .zero)
				}
